...
Rabat_100m_men_rankings_bump_chart <- Rabat_100m_men |>
  ...
  theme(
    ...
    plot.subtitle = element_text(family = "Cabin Sketch", colour = "#362FD9", size = 16, face = "bold",
                                 margin = margin(t = 3, b = 2, unit = "mm"), hjust = 0.5
    ),
    plot.title = element_text(
      face = "bold", colour = "#0e0e0b", family = "Rye", size = 25, hjust = 0.5
    ),
    plot.caption = element_markdown(colour = '#bf820f', hjust = 0.5, size = 11,
  ...
  labs(
    ...
    subtitle = "Rabat 2023: SIMBINE Akani 100m Race Analysis",
    caption = cap
  )

# Save plot
ggsave("SIMBINE_Rabat_100m_men_rankings_2023.png", width = 11, height = 9, bg = "white")

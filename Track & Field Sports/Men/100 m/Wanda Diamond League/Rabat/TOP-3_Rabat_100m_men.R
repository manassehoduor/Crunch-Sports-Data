...
# load libraries
pacman::p_load(ggbump, tidyverse, ggimage, scales, extrafont, ggtext, showtext, htmltools, glue)
...
# load data
load("Rabat_100m_men.rda")

# Define custom athlete colors for bump plot
athlete_colors <- c(
  "KERLEY Fred" = "#8d5406",
  "SIMBINE Akani" = "#362FD9",
  "OMANYALA Ferdinand" = "#101213",
...
Rabat_100m_men_rankings_bump_chart <- Rabat_100m_men |>
  ...
  geom_text(
    data = Rabat_100m_men |>
      filter(Athlete == c("KERLEY Fred", "SIMBINE Akani", "OMANYALA Ferdinand")),
  ...
  geom_text(
    data = Rabat_100m_men |>
      filter(Athlete == c("KERLEY Fred", "SIMBINE Akani", "OMANYALA Ferdinand") & Mark == 100),
    aes(label = glue("{Country}"), col = Athlete), hjust = 0, nudge_x = 12, size = 4.5, 
  ...
  theme(
    ...
    plot.subtitle = element_text(family = "Cabin Sketch", colour = "#362FD9", size = 18, face = "bold",
  ...
  labs(
    ...
    subtitle = "Rabat 2023: Analysis of the three FASTEST Sprinters in the 100m men's race",
    caption = cap
  )

# Save plot
ggsave("Top3_Rabat_100m_men_rankings_2023.png", width = 11, height = 9, bg = "white")

...
# load libraries
pacman::p_load(tidyverse, showtext, ggtext, ggsvg, glue, hms, ggpath, png, jpeg, ggimage, magick)
...
men_100m <- men_100m |>
  mutate(Nationality = toupper(Nationality),
         sec_to_WR = round(Time_PB - 9.58, 2)) |>
  arrange(sec_to_WR)

# Add country logos to the data frame
logo_path <- "C:/Users/ADMIN/Documents/R/Wanda Diamond League/Rabat/men/100m/country_logo/"

# add the resized logos to the data frame
men_100m$logo <- paste0(logo_path, men_100m$Lane, ".png")

# Plot
men_100m |> 
  ggplot(aes(x = Time_PB, y = Lane)) +
  geom_point_svg(aes(Time_PB), svg = svg_txt, size = 16, vjust = 1.3) +
  geom_point_svg(aes(Time_SB), svg = svg_txt2, size = 16, vjust = 1.3) +
  geom_text(data = men_100m|>
            ...
            colour = "black", lineheight = 0.8, nudge_x = 0.05, vjust = 1) +
  geom_text(aes(x = 10.40, label = glue("{Nationality}")),  
            family = 'Roboto Condensed', size = 7,
            colour = "#3d43e6", lineheight = 0.8, nudge_x = 0.05, vjust = 3.7) +
  geom_text(aes(Time_PB, label = glue("{Time_PB}")),  
            family = 'Roboto Condensed', size = 6, fontface = "bold",
            colour = "#190439", lineheight = 0.8, nudge_x = 0.02, vjust = 6) +
  geom_text(aes(Time_SB, label = glue("{Time_SB}")),  
            family = 'Roboto Condensed', size = 6, fontface = "bold",
            colour = "#120ba4", lineheight = 0.8, nudge_x = 0.01, vjust = 1.3) +
  geom_text(data = men_100m|>
              filter(Lane == "Lane 7"),
            aes(Time_SB, label = "World Lead"),  
            family = 'Roboto Condensed', size = 7, fontface = "bold",
            colour = "#120ba4", lineheight = 0.8, nudge_x = 0.01, vjust = 6.1) +
  geom_text(data = men_100m|>
              filter(!Lane == "WORLD RECORD"),
    aes(9.585, label = glue("{sec_to_WR} sec away\nto break WR")),
    family = 'Roboto Condensed', size = 6,
    colour = "#148757", lineheight = 0.8, nudge_x = 0.05, vjust = 1) +
  geom_vline(xintercept = 9.58, linetype = "longdash", size = 2, color = "#7cc121") +
  ...
  theme(
    ...
    plot.title = element_markdown(face = "bold", hjust = 0.5, size = 40, colour = '#291a98'),
    ...
    plot.margin = margin(b = 20, t = 40, r = 0, l = 20)
  ) +
  annotate(
  ...
  annotate(
    geom = "segment", x = 9.77, xend = 9.58, y = "Lane 7", yend = "Lane 7", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#120ba4', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 10.13, xend = 9.58, y = "Lane 1", yend = "Lane 1", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 9.69, xend = 9.58, y = "Lane 2", yend = "Lane 2", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 9.76, xend = 9.58, y = "Lane 3", yend = "Lane 3", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 10.04, xend = 9.58, y = "Lane 4", yend = "Lane 4", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 9.76, xend = 9.58, y = "Lane 5", yend = "Lane 5", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 10.23, xend = 9.58, y = "Lane 6", yend = "Lane 6", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 9.84, xend = 9.58, y = "Lane 8", yend = "Lane 8", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  ) +
  annotate(
    geom = "segment", x = 9.91, xend = 9.58, y = "Lane 9", yend = "Lane 9", 
    arrow = arrow(length = unit(4, "mm")),
    color = '#16985b', size = 2.5
  )

# Save plot
ggsave("Rabat_100m_men1.png", width = 21, height = 17, bg = "white")
